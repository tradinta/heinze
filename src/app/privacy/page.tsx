...
export const metadata: Metadata = {
  title: "Privacy Policy | Robert Heinze",
  description: "Robert Heinze thoughts privacy policy and data governance.",
};
...
export default async function PrivacyPage() {
  let content = "";
  try {
    const res = await db.query("SELECT value FROM system_configs WHERE key = 'page_privacy'");
    if (res.rows.length > 0) {
      content = res.rows[0].value;
    }
  } catch (err) {
    console.error("Failed to query privacy page:", err);
  }
