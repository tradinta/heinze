...
export const metadata: Metadata = {
  title: "Terms of Service | Robert Heinze",
  description: "Robert Heinze thoughts terms of service and website usage rules.",
};
...
export default async function TermsPage() {
  let content = "";
  try {
    const res = await db.query("SELECT value FROM system_configs WHERE key = 'page_terms'");
    if (res.rows.length > 0) {
      content = res.rows[0].value;
    }
  } catch (err) {
    console.error("Failed to query terms page:", err);
  }
